...
class CSFramework_Option_preyantechnosys_one_click_demo_content extends CSFramework_Options {
  ...
  public function output(){
   ...
   wp_enqueue_script(
	...
	// checking if "Minemo Demo Content Setup" plugin is installed
	if( class_exists('preyantechnosys_minemo_one_click_demo_setup') ){
		$button_disabled = "";
	} else {
		$button_disabled = "disabled";
		$error[]         = esc_attr__('The "Minemo Demo Content Setup" plugin is not installed or activated. This plugin is required to Import Demo Content.', 'minemo');
	}
	
	// checking if "Contact Form 7" plugin is installed
	if( !defined('WPCF7_VERSION') ){
		$button_disabled = "disabled";
		$error[]         = esc_attr__('The "Contact Form 7" plugin is not installed or activated. This plugin is required to Import Demo Content. So please install it.', 'minemo');
	}
	...
	<div class="preyantechnosys-one-click-demo-content-wrapper">
		...
		<div class="prt-demo-setup-success-message">
			...
			<?php esc_attr_e('Demo setup done successfully! Now you can start to edit your site', 'minemo'); ?>
		</div>
		<?php endif; ?>
		...
		<div id="import-demo-data-results-wrapper" style="display:none;">
			...
			<?php if( count($error)>0 ){ ?>
				...
				<div class="alert-info prt-one-click-error-message">
					<h4><?php esc_attr_e( 'Please correct the errors given below:', 'minemo' ) ?></h4>
					<ul>
					<?php
					foreach( $error as $line ){
						echo '<li>' . sprintf( esc_attr__('%s ', 'minemo' ) , $line ) . '</li>';
					}
					?>
					</ul>
					<a href="<?php echo esc_url( admin_url( 'themes.php?page=tgmpa-install-plugins' ) ) ?>"><?php esc_attr_e('Click here to install the plugin(s)', 'minemo' ); ?></a> &nbsp; 
					<a href="#" class="prt-one-click-error-close"><?php esc_attr_e('Close', 'minemo' ); ?></a>
				</div>
			...
			<?php } else { ?>
				...
				<?php
				if( function_exists('preyantechnosys_minemo_one_click_html') ){
					preyantechnosys_minemo_one_click_html();
				}
				?>
