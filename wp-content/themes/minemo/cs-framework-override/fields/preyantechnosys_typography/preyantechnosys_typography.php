 ...
 *
 */
class CSFramework_Option_preyantechnosys_typography extends CSFramework_Options {
  ...
  public function output() {
    ...
    $google_json   = preyantechnosys_cs_get_google_fonts();
    ...
    if( is_object( $google_json ) ) {
	  ...
	  echo '<div class="cs-prt-typography-row1">';
	  
	  
	  /**
	   *  Font Family
	   */
	  echo '<div class="prt-option-col-6 cs-prt-typography-dropdown-w cs-prt-typography-family-w">';
	  echo '<label class="prt-typography-family">';
	  echo '<small>'.esc_attr__( 'Font Family', 'minemo' ).'</small><br>';
      ...
      echo '<optgroup label="'. esc_attr__( 'Web Safe Fonts', 'minemo' ) .'">';
      ...
      echo '</optgroup>';
      echo '<optgroup label="'. esc_attr__( 'Google Fonts', 'minemo' ) .'">';
	   ...
	   */
		if( ! empty( $is_backup_family ) ) {
			echo '<div class="prt-option-col-6 cs-prt-typography-dropdown-w cs-prt-typography-bfont-family-w">';
			echo '<label class="prt-cs-backup-family">';
			echo '<small>'.esc_attr__( 'Backup Font Family', 'minemo' ).'</small><br>';
	  ...
	  echo '<div class="clear clr"></div> </div> <!-- .cs-prt-typography-row1 -->';
	  echo '<div class="cs-prt-typography-row2">';
      ...
      if( ! empty( $is_variant ) ) {
		...
		echo '<div class="prt-option-col-6 cs-prt-typography-dropdown-w cs-prt-typography-weight-w">';
        echo '<label class="cs-typography-variant">';
		echo '<small>'.esc_attr__( 'Font Variants', 'minemo' ).'</small><br>';
		...
		if( ! empty( $is_text_transform ) ) {
			$variants = ( $is_google ) ? $googlefonts[$family_value] : $default_variants;
			$variants = ( $value['font'] === 'google' || $value['font'] === 'websafe' ) ? $variants : array( 'regular' );
			echo '<div class="prt-option-col-6 cs-prt-typography-dropdown-w cs-prt-typography-transform-w">';
			echo '<label class="cs-typography-text-transform">';
			echo '<small>'.esc_attr__( 'Text Transform', 'minemo' ).'</small><br>';
	   ...
	   echo '<div class="clear clr"></div>  </div> <!-- .cs-prt-typography-row2 -->';
		...
		if( ! empty( $is_font_size ) ) {
			echo '<div class="prt-fancy-input-w">';
			echo '<label class="cs-typography-font-Size">';
			echo '<small>'.esc_attr__( 'Font Size', 'minemo' ).'</small><br>';
			echo '<div class="input-append">
			<input type="number" class="cs-typo-font-size mini" min="1" max="200" step="1" title="'.esc_attr__( 'Font Size', 'minemo' ).'" name="'. esc_attr($this->element_name( '[font-size]' )) .'" value="'.esc_attr($font_size_value).'">
		...
		if( ! empty( $is_line_height ) ) {
			...
			echo '<small>'.esc_attr__( 'Line Height', 'minemo' ).'</small><br>';
			echo '<div class="input-append">
			<input type="number" class="cs-typo-line-height mini" min="1" max="200" step="1" title="'.esc_attr__( 'Line Height', 'minemo' ).'" name="'. esc_attr($this->element_name( '[line-height]' )) .'" value="'.esc_attr($line_height_value).'">
			<span class="add-on">px</span>
			<div class="clear clr"></div>
			</div><div class="clear clr"></div>';
			echo '</label>';
		...
		if( ! empty( $is_letter_spacing ) ) {
			echo '<div class="prt-fancy-input-w">';
			echo '<label class="cs-typography-line-height">';
			echo '<small>'.esc_attr__( 'Letter Spacing', 'minemo' ).'</small><br>';
			echo '<div class="input-append">
			<input type="number" class="cs-typo-line-height mini" min="0" max="20" step="0.1" title="'.esc_attr__( 'Letter Spacing', 'minemo' ).'" name="'. esc_attr($this->element_name( '[letter-spacing]' )) .'" value="'.esc_attr($letter_spacing_value).'">
			<span class="add-on">px</span>
			<div class="clear clr"></div>
			</div><div class="clear clr"></div>';
			echo '</label>';
		 ...
		 */
		if( ! empty( $is_color ) ) {
			echo '<div class="prt-typography-font-color-w">';
			echo '<label class="prt-typography-font-color">';
			echo '<small>'.esc_attr__( 'Font Color', 'minemo' ).'</small><br>';
      ...
      if( ! empty( $is_all_varients ) ) {
        ...
        $variants = ( $is_google ) ? $googlefonts[$family_value] : $default_variants;
        $variants = ( $value['font'] === 'google' || $value['font'] === 'websafe' ) ? $variants : array( 'regular' );
		echo '<div class="prt-typography-load-all-varient-w">';
        echo '<label class="cs-typography-font-Size">';
		echo '<div class="prt-typo-all-varient-input"><input type="checkbox" name="'. esc_attr($this->element_name( '[all-varients]' )) .'" value"on" '. checked( 'on', $all_varients_value, false ) .' /></div>';
		echo '<div class="prt-typo-load-all-varient-text">'. esc_attr__('Load all variations of the selected font (Like bold, italic etc.)','minemo') .'<br><small>'. esc_attr__('Loading all font variations can slow down (a little) your webpage.','minemo').'</small></div>';
		echo '<div class="clear clr"></div>';
        echo '</label>';
		echo '</div>';
      }
      ...
      echo esc_attr__( 'Error! Can not load json file.', 'minemo' );
 ...
 *
 */
if ( ! function_exists( 'preyantechnosys_cs_get_google_fonts' ) ) {
function preyantechnosys_cs_get_google_fonts() {
	...
	if( ! empty( $tm_cs_google_fonts ) ) {
		...
		ob_start();
		cs_locate_template( 'fields/preyantechnosys_typography/google-fonts.json' );
		$json = ob_get_clean();
