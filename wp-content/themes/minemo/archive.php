	...
	<div id="primary" class="content-area <?php echo preyantechnosys_sanitize_html_classes(preyantechnosys_sidebar_class('content-area')); ?>">
		...
		<?php if ( have_posts() ) : ?>
			...
			if( get_post_type()=='tm_portfolio' ){
				$box               = 'portfolio';
				$global_view       = 'box';
				$global_box_column = preyantechnosys_get_option('pfcat_column');  // one, two, three etc
				$global_box_view   = preyantechnosys_get_option('pfcat_view');  // top-image, left-image etc
				
			} elseif( get_post_type()=='tm_team_member' ){
				$box               = 'team';
				$global_view       = 'box';
				$global_box_column = preyantechnosys_get_option('teamcat_column');  // one, two, three etc
				$global_box_view   = preyantechnosys_get_option('teamcat_view');  // top-image, left-image etc
