 ...
 */
if( ! function_exists( 'minemo_cs_framework_init' ) ) {
function minemo_cs_framework_init(){
	// Codestar framework config
	defined('CS_OPTION'          ) or define('CS_OPTION',           'minemo_theme_options' );
...
add_action( 'init', 'minemo_cs_framework_init', 1 );
add_action( 'admin_init', 'minemo_cs_framework_init', 1 );
...
function preyantechnosys_load_default_theme_options(){
	global $minemo_theme_options;
	
	if( !is_array($minemo_theme_options) ){
		$minemo_theme_options = array();
			...
			if( isset($section['fields']) && is_array($section['fields']) ){
				foreach( $section['fields'] as $field ){
					if( isset($field['id']) && isset($field['default']) ){
						...
						$minemo_theme_options[$field_id] = $field_val;
					}
				}
...
add_action('init','preyantechnosys_load_default_theme_options');
add_action('admin_init','preyantechnosys_load_default_theme_options');

/**
 *  Adding theme fonts
 */
if( ! function_exists( 'preyantechnosys_enqueue_theme_fonts' ) ) {
function preyantechnosys_enqueue_theme_fonts() {
	...
	foreach($options_group as $option_group){
		if( isset($option_group['fields']) && count($option_group['fields'])>0 ){
			...
			foreach($fields as $field){
				if( $field['type'] == 'preyantechnosys_typography' ){
	...
	preyantechnosys_cs_wp_enqueue_scripts($font_elements);
	
}
}
add_filter( 'wp_head', 'preyantechnosys_enqueue_theme_fonts' );

/**
 *  Get all varients of the font
 */
if ( ! function_exists( 'preyantechnosys_get_all_varients' ) ) {
function preyantechnosys_get_all_varients($font){
	$return = array();
	if( function_exists('preyantechnosys_cs_get_google_fonts') ){
		$allfonts = preyantechnosys_cs_get_google_fonts();
...
/**
 *  Load Google fonts for all preyantechnosys_typography elements
 */
if ( ! function_exists( 'preyantechnosys_cs_wp_enqueue_scripts' ) ) {
function preyantechnosys_cs_wp_enqueue_scripts($elements) {
	$all_fonts = array();
	
	// Getting value of all Google Font options
	$google_fonts = preyantechnosys_get_font_option_values($elements);
	...
	foreach( $google_fonts as $google_font ){
		if( isset($google_font['all-varients']) && trim($google_font['all-varients'])=='on' ){
			...
			$all_fonts[ $google_font['family'] ] = preyantechnosys_get_all_varients($google_font['family']);
	...
	if( count($all_fonts)>0 ){
		...
		if( !is_admin() ){
			wp_enqueue_style( 'preyantechnosys-cs-google-fonts', esc_url( '//fonts.googleapis.com/css?family='.$google_font_list ), array(), null );
		}
...
/**
 *  Get value of each preyantechnosys_typography option
 */
if ( ! function_exists( 'preyantechnosys_get_font_option_values' ) ) {
function preyantechnosys_get_font_option_values($elements){
	$google_fonts   = array();
	if( is_array($elements) && count($elements)>0 ){
		global $minemo_theme_options;
		foreach( $elements as $element ){
			$val = ( isset($minemo_theme_options[$element]) ) ? $minemo_theme_options[$element] : array() ;
