...
class Minemo_Historybox_Widget extends Widget_Base{

	public function get_name() {
		return 'tm_historybox_element';
	}

	public function get_title() {
		return esc_attr__( 'History Box', 'minemo' );
	}
	...
	protected function register_controls() {
		...
		$repeater->add_control(
			...
			[
				...
				'placeholder' => esc_attr__( 'Box Sub Title', 'minemo' ),
				'label_block' => true,
			]
		);
...
$this->start_controls_section(
	...
	protected function render() {
		...
		<div class="preyantechnosys-boxes-row-wrapper prt-historybox-wrapper prt-historybox-<?php echo esc_attr($style); ?>">
		<?php
			$col_start_div	= '';
			$col_end_div	= '';
		...
		foreach( $settings['boxes'] as $box ){
			...
			$smalltext_html	= ( !empty($box['smalltext']) ) ? '<div class="prt-historybox-description">'.esc_html($box['smalltext']).'</div>' : '' ;
			...
			if( !empty($box['btn_title']) && !empty($box['btn_link']['url']) ){
				$button_html = '<div class="prt-box-Button-link"><h6>' . tm_link_render($box['btn_link'], 'start' ) . preyantechnosys_wp_kses($box['btn_title']) . tm_link_render($box['btn_link'], 'end' ) . '</h6></div>';
			}
			...
			if( file_exists( locate_template( '/template-parts/historybox/historybox-'.esc_attr($style).'.php', false, false ) ) ){		
			
					$return .= $col_start_div;
					ob_start();
					$r = include( locate_template( '/template-parts/historybox/historybox-'.esc_attr($style).'.php', false, false ) );
...
// Register widget
Plugin::instance()->widgets_manager->register( new Minemo_Historybox_Widget() );