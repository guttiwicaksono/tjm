...
class Minemo_Pricetable_Widget extends Widget_Base{
	...
	public function get_title() {
		return esc_attr__( 'Pricing Table', 'minemo' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'style',
			[
				'label'			=> esc_attr__( 'Select Pricetable Style', 'minemo' ),
				'description'	=> esc_attr__( 'Select Pricetable style.', 'minemo' ),
				'type' => Controls_Manager::SELECT,
				'label_block'	=> true,
				'options' => [
						'style-1' => esc_attr( 'Style 1' ),	
						'style-4' => esc_attr( 'Style 4' ),	
					],
		...
		for( $x=1; $x<=5; $x++ ){
			...
			$this->add_control(
				$x.'_highlight_icon',
				[
					'label'         => esc_attr__( 'Highlight Icon', 'minemo' ),
					'type'          => Controls_Manager::TEXT,
					'description'   => esc_attr__( 'Set i tag code for highlight', 'minemo' ),
					'separator'     => 'after',
					  'condition' => [
						'style' => 'style-4',
					]
		...
		<div class="preyantechnosys-ptables-w prt-ptablebox prt-ptablebox-<?php echo esc_attr($style); ?> <?php echo esc_attr($pricing_table_style); ?>">
			...
			if( !empty($columns) ){
				...
				foreach( $columns as $col => $highlight_col ){
					...
					$heading = ( !empty($settings[$col.'_heading']) ) ? '<div class="prt-ptablebox-title"><h3>'.$settings[$col.'_heading'].'</h3></div>' : '' ;
...
// Register widget
Plugin::instance()->widgets_manager->register( new Minemo_Pricetable_Widget() );