...
class Minemo_Servicebox_Widget extends Widget_Base{
	...
	public function get_title() {
		return esc_attr__( 'Service Box', 'minemo' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'style',
			[
				'label'			=> esc_attr__( 'Select Service Style', 'minemo' ),
		...
		$this->add_control(
			...
			[
				...
				'description' => esc_attr__( 'Sort retrieved service by parameter.', 'minemo' ),
				'type' => Controls_Manager::SELECT,
				'default' => 'DESC',
				'options' => [
		...
		$this->add_control(
			...
			[
				...
				'options' => [
					...
					'55px'		=> esc_attr__( '55px', 'minemo' ),
					'60px'		=> esc_attr__( '60px', 'minemo' ),
					'65px'		=> esc_attr__( '65px', 'minemo' ),
				]
			]
		);
		$this->end_controls_section();
		...
		if ( $posts->have_posts() ) {
			...
			<div class="preyantechnosys-box-heading-wrapper <?php echo preyantechnosys_wp_kses($headingclass); ?>">
				...
				if( function_exists('preyantechnosys_get_filterbutton') ){
					$sortable_category_html = preyantechnosys_get_filterbutton( $settings, 'tm_service_category' );
			...
			while ( $posts->have_posts() ) {
				...
				if( file_exists( locate_template( '/template-parts/servicebox/servicebox-'.esc_attr($style).'.php', false, false ) ) ){
					...
					ob_start();
					$r = include( locate_template( '/template-parts/servicebox/servicebox-'.esc_attr($style).'.php', false, false ) );
...
// Register widget
Plugin::instance()->widgets_manager->register( new Minemo_Servicebox_Widget() );