...
function preyantechnosys_header_style_change_triggers(){
	...
	var infostack_column_one		  = '<div class="header-icon"> <div class="icon"><i class="kw_minemo flaticon-phone-call"></i></div></div><div class="header-content"><h3>Telephone</h3><h5>+123 795 9841</h5></div>';
	var infostack_column_two		  = '<div class="header-icon"> <div class="icon"><i class="kw_minemo flaticon-big-envelope"></i></div></div><div class="header-content"><h3>Email Address</h3><h5>info@example.com</h5></div>';
	var infostack_column_three		  = '<div class="header-icon"> <div class="icon"><i class="kw_minemo flaticon-pin"></i></div></div><div class="header-content"><h3>Office Address</h3><h5> 23 Belfast ave, Florida</h5></div>';
	
	jQuery('input[name="minemo_theme_options[headerstyle]"]').change(function() {
		...
		if (this.value == 'classic') { // Classic
			jQuery('select[name="minemo_theme_options[header_bg_color]"]').val(header_bg_color).change();
			jQuery('input[name="minemo_theme_options[titlebar_background][color]"]').iris('color', titlebar_background_color);  // 
			jQuery('select[name="minemo_theme_options[titlebar_text_color]"]').val(titlebar_text_color);
			jQuery('input[name="minemo_theme_options[titlebar_height]"').val(titlebar_height);  // Titlebar height
			jQuery('input[name="minemo_theme_options[show_topbar]"]').prop('checked', true);  // Show topbar
			jQuery('input[name="minemo_theme_options[header_search]"]').prop('checked', false);  // Show header search
			jQuery('input[name="minemo_theme_options[mainmenufont][color]"').iris('color', mainmenufont_color);  // 
			jQuery('input[name="minemo_theme_options[header_height]"]').val(header_height);  // Header Height
			jQuery('input[name="minemo_theme_options[logoimg][id]"]').val('');
			jQuery('input[name="minemo_theme_options[logoimg][thumb-url]"]').val( imgurl+logoimg );
			jQuery('input[name="minemo_theme_options[logoimg][full-url]').val( imgurl+logoimg );
			jQuery('input[name="minemo_theme_options[logo_max_height]"]').val(logo_max_height);  //
			jQuery('select[name="minemo_theme_options[mainmenu_active_link_color]"]').val(mainmenu_active_link_color).change(); // Select
			jQuery('input[name="minemo_theme_options[stickymainmenufontcolor]"]').iris('color', stickymainmenufontcolor_color);  // 
			jQuery('select[name="minemo_theme_options[sticky_header_bg_color]"]').val(sticky_header_bg_color).change();		
			jQuery('input[name="minemo_theme_options[dropmenu_background][color]"]').iris('color', dropmenu_background_color);  // Dropdown Menu color
			jQuery('input[name="minemo_theme_options[dropdownmenufont][color]"]').iris('color', dropdownmenufont_color);  // Dropdown Menu color
			jQuery('textarea[name="minemo_theme_options[header_text]"]').val(header_text);  //
...
function preyantechnosys_show_hide_titlebar_textcolor(){
	...
	var $this      = jQuery( 'select[name="minemo_theme_options[titlebar_text_color]"]' );
	var $page_this = jQuery( 'select[name="_preyantechnosys_metabox_group[titlebar_text_color]"]' );
	
	if( $this.length > 0 ){
		if( jQuery($this).val()=='custom' ){
			jQuery( 'input[name="minemo_theme_options[titlebar_heading_font][color]"]' ).closest('.prt-typography-font-color-w').show();
			jQuery( 'input[name="minemo_theme_options[titlebar_subheading_font][color]"]' ).closest('.prt-typography-font-color-w').show();
			jQuery( 'input[name="minemo_theme_options[titlebar_breadcrumb_font][color]"]' ).closest('.prt-typography-font-color-w').show();
		} else {
			jQuery( 'input[name="minemo_theme_options[titlebar_heading_font][color]"]' ).closest('.prt-typography-font-color-w').hide();
			jQuery( 'input[name="minemo_theme_options[titlebar_subheading_font][color]"]' ).closest('.prt-typography-font-color-w').hide();
			jQuery( 'input[name="minemo_theme_options[titlebar_breadcrumb_font][color]"]' ).closest('.prt-typography-font-color-w').hide();
		}
	}
...
function preyantechnosys_show_hide_color_picker_background(){
	...
	jQuery(items).each(function(n,val){
		...
		var $dropdown_element   = jQuery( 'select[name="minemo_theme_options['+dropdown_name+']"]' );
		
		// show/hide the color picker on load
		if( $dropdown_element.val()=='custom' ){
			jQuery( 'input[name="minemo_theme_options['+background_name+'][color]"]' ).closest('.prt-background-color-w').show();
		} else {
			jQuery( 'input[name="minemo_theme_options['+background_name+'][color]"]' ).closest('.prt-background-color-w').hide();
		}
...
jQuery(document).ready( function() {
	...
	jQuery( 'select[name="minemo_theme_options[titlebar_text_color]"]' ).change(function() {  // Theme Options
		preyantechnosys_show_hide_titlebar_textcolor();
	});
	...
	jQuery('input[type=radio][name="minemo_theme_options[loaderimg]"]:checked').closest('label').addClass('prt-pre-loader-option-selected');
	jQuery('input[type=radio][name="minemo_theme_options[loaderimg]"]').change(function() {
		jQuery('input[type=radio][name="minemo_theme_options[loaderimg]"]').closest('label').removeClass('prt-pre-loader-option-selected');
