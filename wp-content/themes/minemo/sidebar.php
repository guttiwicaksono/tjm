...
if( is_page() ){
	...
	$sidebar_right_page = get_post_meta($post->ID,'_preyantechnosys_page_options_rightsidebar',true);
	...
	if( function_exists('tribe_is_upcoming') ){
		if (get_post_type()=='tribe_events'){
			$events_sidebar = !empty($minemo_theme_options['sidebar_events']) ? esc_attr($minemo_theme_options['sidebar_events']) : 'no' ; // Global settings
	...
	$sidebar_right_blog = get_post_meta( $pageid ,'_preyantechnosys_'.$postType.'_options_rightsidebar',true);
	...
	if( function_exists('tribe_is_upcoming') ){
		if ( get_post_type() == 'tribe_events' || tribe_is_upcoming() || tribe_is_month() || tribe_is_by_date() || tribe_is_day() || is_single('tribe_events')){
			$events_sidebar = ( isset($minemo_theme_options['sidebar_events']) && trim($minemo_theme_options['sidebar_events'])!='' ) ? esc_attr($minemo_theme_options['sidebar_events']) : 'no' ; // Global settings
			if( $events_sidebar=='right' ){
				$sidebar_right = 'sidebar-events';
			}
		}
	...
	$sidebar_right = esc_attr($minemo_theme_options['sidebar_post']); // Global settings
	$sidebar_right = 'sidebar-right-blog';
	$sidebar_right_post = get_post_meta($post->ID,'_preyantechnosys_post_options_rightsidebar',true);
