<?php
/**
 * Minemo functions and definitions
 *
...
function preyantechnosys_setup() {
	 ...
	 * If you're building a theme based on minemo, use a find and replace
	 * to change 'minemo' to the name of your theme in all the template files
	 */
	load_theme_textdomain( 'minemo', get_template_directory() . '/languages' );
	...
	register_nav_menus( array( // https://codex.wordpress.org/Function_Reference/has_nav_menu
		'preyantechnosys-main-menu'			=> esc_html__( 'Main Menu', 'minemo' ),
	) );
	
	if( function_exists('preyantechnosys_minemo_cs_framework_init')){
		register_nav_menus( array( // https://codex.wordpress.org/Function_Reference/has_nav_menu
			'preyantechnosys-footer-menu'		=> esc_html__( 'Footer Menu', 'minemo' ),
		) );
	}
...
add_action('wp_loaded', 'minemo_prefix_output_buffer_start');
function minemo_prefix_output_buffer_start() { 
    ob_start("minemo_prefix_output_callback"); 
}
function minemo_prefix_output_callback($buffer) {
    return preg_replace( "%[ ]type=[\'\"]text\/(javascript|css)[\'\"]%", '', $buffer );
}


if( ! defined( 'PRT_ENVATO_API' ) ) define( 'PRT_ENVATO_API', esc_url('https://preyantechnosys.com/envato-api/verify-purchase.php'));
