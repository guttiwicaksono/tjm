...
if( !empty($minemo_theme_options['fbar_widget_column_layout']) ){
	$floatingbar_col = esc_attr($minemo_theme_options['fbar_widget_column_layout']);
}
?>

<div id="floatingbar-widgets" class="row multi-columns-row sidebar-container floatingbar-text-color-<?php echo sanitize_html_class($minemo_theme_options['fbar_text_color']); ?>" role="complementary">
...
if($floatingbar_col == '3_3_3_3'){
	...
	if( is_array($floatingbar_col) && count($floatingbar_col)>0 ){
		...
		foreach($floatingbar_col as $col){
			...
			<div class="widget-area <?php echo preyantechnosys_sanitize_html_classes($row_class); ?> first-widget-area">
