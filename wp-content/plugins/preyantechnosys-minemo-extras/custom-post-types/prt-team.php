...
function preyantechnosys_minemo_cpt_tm_team(){

	// Getting Options
	$minemo_theme_options = get_option('minemo_theme_options');
	
	$team_type_title          = ( !empty($minemo_theme_options['team_type_title']) ) ? $minemo_theme_options['team_type_title'] : 'Team Members' ;
	...
	$team_type_slug           = ( !empty($minemo_theme_options['team_type_slug']) ) ? $minemo_theme_options['team_type_slug'] : 'team-member' ;
	
	$team_group_title          = ( !empty($minemo_theme_options['team_group_title']) ) ? $minemo_theme_options['team_group_title'] : 'Team Groups' ;
	$team_group_title_singular = ( !empty($minemo_theme_options['team_group_title_singular']) ) ? $minemo_theme_options['team_group_title_singular'] : 'Team Group' ;
	$team_cat_slug           = ( !empty($minemo_theme_options['team_cat_slug']) ) ? $minemo_theme_options['team_cat_slug'] : 'team-group' ;
	...
	add_action('do_meta_boxes', 'preyantechnosys_minemo_tm_team_featured_image_box');
	function preyantechnosys_minemo_tm_team_featured_image_box() {
...
add_action( 'init', 'preyantechnosys_minemo_cpt_tm_team', 8 );
...
add_filter( 'manage_tm_team_member_posts_columns', 'preyantechnosys_tm_team_member_set_featured_image_column' );
add_action( 'manage_tm_team_member_posts_custom_column' , 'preyantechnosys_tm_team_member_set_featured_image_column_content', 10, 2 );
if ( ! function_exists( 'preyantechnosys_tm_team_member_set_featured_image_column' ) ) {
function preyantechnosys_tm_team_member_set_featured_image_column($columns) {
	$new_columns = array();
...
if ( ! function_exists( 'preyantechnosys_tm_team_member_set_featured_image_column_content' ) ) {
function preyantechnosys_tm_team_member_set_featured_image_column_content( $column, $post_id ) {
 ...
 *  Meta Box: Team
 */
if ( ! function_exists( 'preyantechnosystmte_minemo_tm_team_metabox_options' ) ) {
function preyantechnosystmte_minemo_tm_team_metabox_options( $options ) {
	...
	$team_extra_details_lines  = ( !empty($minemo_theme_options['team_extra_details_lines']) ) ? $minemo_theme_options['team_extra_details_lines'] : array() ;
	...
	$list_array = array(
		array(
		 ...
		 array (
			...
			"type"  => "text",
			"title" => '<span class="prt-admin-team-list-icon"> <i class="fa fa-pencil"></i></span> &nbsp; '.__('Position', 'tmte'),
		),
		array (
			"id"    => "team_details_line_email",
			"type"  => "text",
			"title" => '<span class="prt-admin-team-list-icon"> <i class="fa fa-envelope"></i></span> &nbsp; '.__('Email', 'tmte'),
		),
		array(
			"id"    => "team_details_line_phone",
			"type"  => "text",
			"title" => '<span class="prt-admin-team-list-icon"> <i class="fa fa-phone"></i></span> &nbsp; '.__('Phone', 'tmte'),
		),
		array(
			"id"    => "team_details_line_website",
			"type"  => "text",
			"title" => '<span class="prt-admin-team-list-icon"> <i class="fa fa-link"></i></span> &nbsp; '.__('Website', 'tmte'),
		)
	);
	...
	if( is_array($team_extra_details_lines) && count($team_extra_details_lines) > 0 ){
		...
		foreach( $minemo_theme_options['team_extra_details_lines'] as $key=>$val ){
			
	
			$this_array = array();
			$this_array['id']    = 'team_extra_details_line_'.$key;
			$this_array['type']  = 'text';
			$this_array['title'] = '<span class="prt-admin-team-list-icon"></span> &nbsp; '. esc_attr__($val['team_extra_details_line_title'], 'minemo');
	...
	$options[]    = array(
		'id'        => 'preyantechnosys_team_member_details',
		'title'     => sprintf( esc_attr__("Minemo: %s's Details", 'tmte'), $team_type_title_singular ),
		'post_type' => 'tm_team_member', // only here is important
		'context'   => 'normal',
		'priority'  => 'default',
		'sections'  => array(
			array(
				'name'   => 'preyantechnosys_team_list_data',
				'fields' => array(
					array(
						'id'        => 'tm_team_info',
						'type'      => 'fieldset',
	...
	// Team Members Details
	$options[]    = array(
		'id'        => 'preyantechnosys_team_member_social',
		'title'     => sprintf( esc_attr__("Minemo: %s's Social Links", 'tmte'), $team_type_title_singular ),
		'post_type' => 'tm_team_member', // only here is important
		'context'   => 'normal',
		'priority'  => 'default',
		'sections'  => array(
			...
			array(
				'name'   => 'preyantechnosys_team_socials',
				...
				'fields' => array(
					array(
						...
						'info'            => esc_attr__('Add your social services here. Also you can reorder the Social Links as per your choice. Just drag and drop items to reorder and save settings', 'minemo'),
						'button_title'    => esc_attr__('Add New Social Links', 'minemo'),
						'accordion_title' => 'social_icons_list_icon',
						'fields'          => array(
							array(
								'id'            => 'social_icons_list_icon',
								'type'          => 'select',
								'title'         =>  esc_attr__('Social Sevice', 'minemo'),
								'options'  		=> array(
													...
													'rss'        => esc_attr__('RSS', 'minemo' ),
												),
								'class'         => 'chosen',
								'default'       => 'twitter',
								'after'  		=> '<div class="cs-text-muted"><br>'.__('Select Social service from here', 'minemo').'</div>',
							),
							array(
								'id'     		=> 'social_icons_list_link',
								'type'    		=> 'text',
								'title'   		=> esc_attr__('Link to Social service selected above', 'minemo'),
...
add_filter( 'cs_metabox_options', 'preyantechnosystmte_minemo_tm_team_metabox_options' );
