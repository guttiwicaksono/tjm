...
function preyantechnosys_minemo_cpt_tm_service(){

	$minemo_theme_options = get_option('minemo_theme_options');
	
	$service_type_title          = ( !empty($minemo_theme_options['service_type_title']) ) ? $minemo_theme_options['service_type_title'] : 'Service' ;
	...
	$service_type_slug           = ( !empty($minemo_theme_options['service_type_slug']) ) ? $minemo_theme_options['service_type_slug'] : 'service' ;
	
	$service_cat_title          = ( !empty($minemo_theme_options['service_cat_title']) ) ? $minemo_theme_options['service_cat_title'] : 'Service Categories' ;
	$service_cat_title_singular = ( !empty($minemo_theme_options['service_cat_title_singular']) ) ? $minemo_theme_options['service_cat_title_singular'] : 'Service Category' ;
	$service_cat_slug           = ( !empty($minemo_theme_options['service_cat_slug']) ) ? $minemo_theme_options['service_cat_slug'] : 'service-category' ;
...
add_action( 'init', 'preyantechnosys_minemo_cpt_tm_service', 8 );

// Show Featured image in the admin section
add_filter( 'manage_tm_service_posts_columns', 'preyantechnosys_tm_service_set_featured_image_column' );
add_action( 'manage_tm_service_posts_custom_column' , 'preyantechnosys_tm_service_set_featured_image_column_content', 10, 2 );
if ( ! function_exists( 'preyantechnosys_tm_service_set_featured_image_column' ) ) {
function preyantechnosys_tm_service_set_featured_image_column($columns) {
	$new_columns = array();
...
if ( ! function_exists( 'preyantechnosys_tm_service_set_featured_image_column_content' ) ) {
function preyantechnosys_tm_service_set_featured_image_column_content( $column, $post_id ) {
 ...
 *  Meta Boxes: Service
 */
if ( ! function_exists( 'preyantechnosys_minemo_tm_service_metabox_options' ) ) {
function preyantechnosys_minemo_tm_service_metabox_options( $options ) {
	...
	$list_array    = array();
	$options_array = array();
	if( isset($minemo_theme_options['service_details_line']) && is_array($minemo_theme_options['service_details_line']) && count( $minemo_theme_options['service_details_line'] ) > 0 ){
		foreach( $minemo_theme_options['service_details_line'] as $key=>$val ){
			...
			$option_array = array(
				...
				'type'       => 'text',
				'title'      => '<span class="prt-admin-service-list-icon"> <i class="'. $icon_class .'"></i></span> &nbsp; '. esc_attr__($val['service_details_line_title'], 'minemo'),
			);
	...
	$options[]    = array(
		'id'        => 'preyantechnosys_service_featured',
		...
		'sections'  => array(
			array(
				'name'   => 'preyantechnosys_service_featured',
				'fields' => array(
...
	$options[]    = array(		'id'        => 'preyantechnosys_prtservice_icon',		'title'     => esc_attr__('Minemo: Service icon', 'tmte'),		'post_type' => 'tm_service', // only here is important		'context'   => 'normal',		'priority'  => 'default',		'sections'  => array(			array(				'name'   => 'preyantechnosys_service_icon',				'fields' => array(							array(						'id'     		=> 'prt_serviceicon',						'type'    		=> 'textarea',						'title'   		=> esc_attr__('Service Icon', 'tmte'),						'after'  		=> '<div class="cs-text-muted"><br>'.__("(Optional) Please select icon for Servicebox element", 'tmte').'</div>',					),				),			),		),);
	return $options;
}
}
add_filter( 'cs_metabox_options', 'preyantechnosys_minemo_tm_service_metabox_options' );