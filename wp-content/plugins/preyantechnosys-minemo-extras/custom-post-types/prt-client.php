...
function preyantechnosys_minemo_cpt_tm_client(){
	...
	function preyantechnosys_minemo_tm_client_enter_title_here( $title ){
	...
	add_filter( 'enter_title_here', 'preyantechnosys_minemo_tm_client_enter_title_here' );




	// Move Featured Image box from left to center only on CLIENTS custom_post_type
	add_action('do_meta_boxes', 'preyantechnosys_minemo_tm_client_featured_image_box');
	function preyantechnosys_minemo_tm_client_featured_image_box() {
...
add_action( 'init', 'preyantechnosys_minemo_cpt_tm_client', 8 );
...
add_filter( 'manage_tm_client_posts_columns', 'preyantechnosys_tm_client_set_featured_image_column' );
add_action( 'manage_tm_client_posts_custom_column' , 'preyantechnosys_tm_client_set_featured_image_column_content', 10, 2 );
if ( ! function_exists( 'preyantechnosys_tm_client_set_featured_image_column' ) ) {
function preyantechnosys_tm_client_set_featured_image_column($columns) {
	$new_columns = array();
...
if ( ! function_exists( 'preyantechnosys_tm_client_set_featured_image_column_content' ) ) {
function preyantechnosys_tm_client_set_featured_image_column_content( $column, $post_id ) {
 ...
 *  Meta Box: Clients
 */
if ( ! function_exists( 'preyantechnosys_minemo_tm_client_metabox_options' ) ) {
function preyantechnosys_minemo_tm_client_metabox_options( $options ) {
	

	
	// Client Details Meta Box
	$options[]    = array(
		'id'        => 'preyantechnosys_clients_details',
		'title'     => esc_attr__('Minemo: Client Details', 'tmte'),
		'post_type' => 'tm_client', // only here is important
		'context'   => 'normal',
		'priority'  => 'default',
		'sections'  => array(
			array(
				'name'   => 'preyantechnosys_clients',
				'fields' => array(
...
if( function_exists('cs_framework_init') ){
	add_filter( 'cs_metabox_options', 'preyantechnosys_minemo_tm_client_metabox_options' );
}