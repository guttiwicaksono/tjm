...
// [prt-btn type="fontawesome" size="small" bgcolor="grey" align="center" roundborder="yes"]
if( !function_exists('preyantechnosys_sc_btn') ){
function preyantechnosys_sc_btn( $atts, $content=NULL ) {
	...
	if( function_exists('vc_map') ){
		...
		$options_list   = preyantechnosys_create_options_list($tm_sc_params_btn);
		...
		//parse link
		$link = ( '||' === $link ) ? '' : $link;
		$link = preyantechnosys_vc_build_link( $link );
		$use_link = false;
		...
		// CSS classes in DIV
		$css_class   = array();
		$css_class[] = 'prt-vc_btn3-container';
		...
		if( !empty($css_animation) ){
			$css_animation = preyantechnosys_getCSSAnimation( $css_animation );
		...
		$wrapper_classes = array(
			'prt-vc_btn3-container',
			preyantechnosys_getExtraClass( $el_class ),
			preyantechnosys_getCSSAnimation( $css_animation ),
			'prt-vc_btn3-' . $align,
		);
		...
		$button_classes = array(
			'prt-vc_general',
			'prt-vc_btn3',
			'prt-vc_btn3-size-' . $size,
			'prt-vc_btn3-shape-' . $shape,
			'prt-vc_btn3-style-' . $style,
			'prt-vc_btn3-weight-' . $font_weight,
		);
		
		
		// Text style - adding outline class to set all text color
		if( $style=='text' ){
			//$button_classes[] = 'prt-vc_btn3-style-outline';
		}
		...
		$button_html = $title;
		if ( '' === trim( $title ) ) {
			$button_classes[] = 'prt-vc_btn3-o-empty';
			$button_html = '<span class="prt-vc_btn3-placeholder">&nbsp;</span>';
		}
		
		
		
		if ( 'true' === $button_block && 'inline' !== $align ) {
			$button_classes[] = 'prt-vc_btn3-block';
		}
		if ( 'true' === $add_icon ) {
			$button_classes[] = 'prt-vc_btn3-icon-' . $i_align;
			preyantechnosys_vc_icon_element_fonts_enqueue( $i_type );

			if ( isset( ${'i_icon_' . $i_type} ) ) {
				if ( 'pixminemoons' === $i_type ) {
					$icon_wrapper = true;
				}
			...
			if ( $icon_wrapper==true ) {
				$icon_html = '<i class="prt-vc_btn3-icon"><span class="prt-vc_btn3-icon-inner ' . esc_attr( $icon_class ) . '"></span></i>';
			} else {
				$icon_html = '<i class="prt-vc_btn3-icon ' . esc_attr( $icon_class ) . '"></i>';
			}
		...
		if ( 'custom' === $style ) {
			...
			if ( ! $custom_background && ! $custom_text ) {
				$button_classes[] = 'prt-vc_btn3-color-grey';
			}
			...
			if ( ! $outline_custom_color && ! $outline_custom_hover_background && ! $outline_custom_hover_text ) {
				$button_classes[] = 'prt-vc_btn3-color-inverse';

				foreach ( $button_classes as $k => $v ) {
					if ( 'prt-vc_btn3-style-outline-custom' === $v ) {
			...
			$uid = uniqid();
			$return .= '<style type="text/css">.prt-vc_btn3-style-' . $style . '.prt-vc_btn-gradient-btn-' . $uid . ':hover{' . implode( ';',
					$gradient_css_hover ) . ';' . '}</style>';
			$return .= '<style type="text/css">.prt-vc_btn3-style-' . $style . '.prt-vc_btn-gradient-btn-' . $uid . '{' . implode( ';',
					$gradient_css ) . ';' . '}</style>';
			$button_classes[] = 'prt-vc_btn-gradient-btn-' . $uid;
			$attributes[] = 'data-vc-gradient-1="' . $gradient_color_1 . '"';
			$attributes[] = 'data-vc-gradient-2="' . $gradient_color_2 . '"';
		} else {
			$button_classes[] = 'prt-vc_btn3-color-' . $color;
		}
...
add_shortcode( 'prt-btn', 'preyantechnosys_sc_btn' );