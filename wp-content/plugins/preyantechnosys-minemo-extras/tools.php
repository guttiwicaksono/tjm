	...
	if( $logotype=='image' ){
		if( isset($logoimg) && is_array($logoimg) ){
			...
			if( isset($logoimg['full-url']) && trim($logoimg['full-url'])!='' ){
				...
				$return .= '<img class="preyantechnosys-logo-img standardlogo" src="'.$logoimg['full-url'].'" >';
			
			} else if( isset($logoimg['thumb-url']) && trim($logoimg['thumb-url'])!='' ){
				$image = $logoimg['thumb-url'];
				$return .= '<img class="preyantechnosys-logo-img standardlogo" src="'.$logoimg['thumb-url'].'" >';

			} else if( isset($logoimg['id']) && trim($logoimg['id'])!='' ){
				$image   = wp_get_attachment_image_src( $logoimg['id'], 'full' );
				$return .= '<img class="preyantechnosys-logo-img standardlogo" src="'.$image[0].'" width="'.$image[1].'" height="'.$image[2].'">';
			...
			if( isset($logoimg_sticky) && is_array($logoimg_sticky) ){
				...
				if( isset($logoimg_sticky['full-url']) && trim($logoimg_sticky['full-url'])!='' ){
					...
					$return .= '<img class="preyantechnosys-logo-img stickylogo" src="'.$logoimg_sticky['full-url'].'" alt="" >';
				
				} else if( isset($logoimg_sticky['thumb-url']) && trim($logoimg_sticky['thumb-url'])!='' ){
					$sticky_image   = $logoimg_sticky['thumb-url'];
					$return .= '<img class="preyantechnosys-logo-img stickylogo" src="'.$logoimg_sticky['thumb-url'].'" alt="" >';
				
				} else if( isset($logoimg_sticky['id']) && trim($logoimg_sticky['id'])!='' ){
					$sticky_image   = wp_get_attachment_image_src( $logoimg_sticky['id'], 'full' );
					$return .= '<img class="preyantechnosys-logo-img stickylogo" src="'.$sticky_image[0].'" width="'.$sticky_image[1].'" height="'.$image[2].'" alt="">';
...
function preyantechnosys_pagination( $wp_query_data=false ){
	...
	$paginateLinks = paginate_links( array(
		...
		'type'      => 'array',
		'prev_text' => '<i class="prt-minemo-icon-arrow-left"></i> <span class="prt-hide prt-pagination-text prt-pagination-text-prev">' . esc_attr__( 'Previous page', 'minemo' ) . '</span>',
		'next_text' => '<span class="prt-hide prt-pagination-text prt-pagination-text-next">' . esc_attr__( 'Next page', 'minemo' ) . '</span> <i class="prt-minemo-icon-arrow-right"></i>',
	) );
...
function minemo_entry_meta( $metafor="blogbox" ) {
			...
			switch( $meta_id ){
				...
				case 'date':
					...
					if ( in_array( get_post_type(), array( 'post', 'attachment' ) ) ) {
						...
						$return .= sprintf( '<span class="prt-meta-line posted-on"><i class="prt-minemo-icon-clock"></i> <span class="screen-reader-text prt-hide">%1$s </span><a href="%2$s" rel="bookmark">%3$s</a></span>',
				...
				case 'author':
					...
					if ( 'post' === get_post_type() ) {
						...
						$return .= sprintf( '<span class="prt-meta-line byline"><i class="prt-minemo-icon-user"></i> <span class="author vcard"><span class="screen-reader-text prt-hide">%1$s </span>%2$s</span></span>',
				...
				case 'comment':
					...
					if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
						$return .= '<span class="prt-meta-line comments-link"><i class="prt-minemo-icon-comments-smiley"></i> ';
						ob_start();
						comments_popup_link( esc_attr__( 'Leave a comment', 'minemo' ) );
				...
				case 'tag':
					...
					if ( !empty($tags_list) ) {
						...
						$return .= sprintf( '<span class="prt-meta-line tags-links"><i class="prt-minemo-icon-tag"></i> <span class="screen-reader-text prt-hide" prt-hide>%1$s </span>%2$s</span>',
	...
	// Social share
	if( $metafor != "blogbox" ){
		$social_share = preyantechnosys_social_share_box('post');
	}
	
	// meta details
	if( !empty($return) ){
		$return = '<div class="prt-entry-meta-wrapper"><div class="entry-meta prt-entry-meta prt-entry-meta-' . $metafor . '">' . $return . '</div>' . $social_share .  '</div>' ;
	}
...
function preyantechnosys_get_background_css( $element_array, $values, $exclude=array() ){
	...
	if( count($return)>0 ){
		...
		if( /*$dropdown_color=='custom' &&*/ !in_array('output_bglayer',$exclude) && is_array($return_bglayer) && count($return_bglayer)>0  ){
			if( $selector!='' ){
				$return .= $selector.' > .prt-bg-layer{'.implode( ';', $return_bglayer ).';}'."\n";
			}
		}
...
function preyantechnosys_get_all_background_css( $echo=true ){
	$return = array();
...
function preyantechnosys_blogbox_comment_count( $echo = false ) {
	$return = '';
	if( comments_open() ){
		//$return .= '<div class="preyantechnosys-blogbox-footer-right preyantechnosys-wrap-cell">';
...
function preyantechnosys_featured_image($size='full'){
	...
	*/
	...
	if( !empty($return) ){
		$return = '
		<div class="preyantechnosys-item-thumbnail">
			<div class="preyantechnosys-item-thumbnail-inner">
				' . $return . '
			</div>
		</div>';
	}
...
if ( !function_exists( 'preyantechnosys_team_member_single_view' ) ){
function preyantechnosys_team_member_single_view() {
	$view = '';
	$team_member_viewstyle = preyantechnosys_get_option('team_member_viewstyle');
	...
	$single_viewstyle = get_post_meta( get_the_ID(), 'preyantechnosys_team_member_single_view', true );
...
function preyantechnosys_team_member_single_meta( $type='position', $post_id='' ) {
	...
	if( !empty($type) ){
		...
		if( !empty($return) ){
			...
			switch( $type ){
				case 'position':
					$return = '<h5 class="prt-team-member-single-position">' . $return . '</h5>';
					break;
					
				case 'phone':
					$return = '<div class="prt-team-list-title prt-skincolor"><i class="prt-minemo-icon-phone"></i> '. esc_attr__('Phone','minemo') .'</div>
						<div class="prt-team-list-value"><a href="tel:' . $return . '">' . $return . '</a></div>';
					break;
					
				case 'email':
					$return = '<div class="prt-team-list-title prt-skincolor"><i class="prt-minemo-icon-mail"></i> '. esc_attr__('Email','minemo') .'</div>
				...
				case 'website':
					...
					$return = '<div class="prt-team-list-title prt-skincolor"><i class="prt-minemo-icon-world"></i> '. esc_attr__('Website','minemo') .'</div>
						<div class="prt-team-list-value"><a target="_blank" href="' . $return_link . '">' . $return . '</a></div>';
					break;
	...
	if( !empty($team_extra_details_lines) && is_array($team_extra_details_lines) && count($team_extra_details_lines) > 0 ){
		...
		foreach( $team_extra_details_lines as $key=>$val ){
			...
			if( !empty($post_meta['tm_team_info']['team_extra_details_line_'.$key]) ){
				$icon  = preyantechnosys_create_icon_from_data( $val['team_extra_details_line_icon'], true );
				...
				$icon_none = ( !empty($icon) ) ? '' : 'prt-list-detail-no-icon' ;
				
				$return .= '<li class="prt-team-details-line prt-team-extra-details-line-' . $key . ' ' . $icon_none . '">
					<div class="prt-team-list-title prt-skincolor">' . $icon . $title . '</div>
...
function preyantechnosys_create_icon_from_data( $data, $i_tag_only=false ){
	...
	if( isset($data['library']) && isset($data['library_fontawesome']) && isset($data['library_linecons']) && isset($data['library_themify']) ){
		...
		if( !empty($$library) ){
			...
			$return = do_shortcode('[prt-icon type="'.$library.'" icon_fontawesome="'.$library_fontawesome.'" icon_linecons="'.$library_linecons.'" icon_themify="'.$library_themify.'"]');
...
function preyantechnosys_team_member_content( $post_id='' ){
	...
	$return = '<div class="prt-team-member-content">' . $content . '</div><!-- .prt-team-member-content -->';
...
function preyantechnosys_portfolio_next_prev_btn() {
	...
	if( is_singular('tm_portfolio') ){
		...
		$return = get_the_post_navigation( array(
			'next_text' => '<span class="meta-nav prt-hide" aria-hidden="true">' . __( 'Next', 'minemo' ) . '</span> ' .
				'<span class="screen-reader-text prt-hide">' . __( 'Next post:', 'minemo' ) . '</span> ' .
				'<span class="post-title prt-hide">%title</span>',
			'prev_text' => '<span class="meta-nav prt-hide" aria-hidden="true">' . __( 'Previous', 'minemo' ) . '</span> ' .
				'<span class="screen-reader-text prt-hide">' . __( 'Previous post:', 'minemo' ) . '</span> ' .
				'<span class="post-title prt-hide">%title</span>',
		) );
...
function preyantechnosys_portfolio_detailsbox(){
	...
	if( isset($pf_details_line) && is_array($pf_details_line) && count($pf_details_line)>0 ){
		foreach( $pf_details_line as $key=>$val ){
			...
			if( !empty($val['pf_details_line_title']) ){ $row_title = sprintf( __('%s', 'minemo'), $val['pf_details_line_title'] ); }
...
/**
 *  Blogbox class : preyantechnosys_portfoliobox_class()
 */
...
function preyantechnosys_portfoliobox_media_link( $default_icon='prt-minemo-icon-search', $video_icon='prt-minemo-icon-video', $audio_icon='prt-minemo-icon-music-alt', $slider_icon='prt-minemo-icon-gallery-1' ){

	$icon_code='<i class="prt-minemo-icon-search"></i>';
	...
	// default output
	$return = '<a class="tm_prettyphoto" title="' . get_the_title() . '" href="' . esc_url(preyantechnosys_portfolio_single_image_path()) . '">' . $icon_code . '</a>';
	...
	if( !empty($portfolio_featured['featuredtype']) ){
		...
		switch( $portfolio_featured['featuredtype'] ){
			...
			case 'slider':
				...
				$return = '<a title="' . get_the_title() . '" class="preyantechnosys-open-gallery" data-id="' . get_the_ID() . '" href="#preyantechnosys-embed-code-' . get_the_ID() . '">' . $icon_code . '</a>';
			...
			case 'audioembed':
				...
				$return = '<a title="' . get_the_title() . '" class="tm_prettyphoto" href="#preyantechnosys-embed-code-' . get_the_ID() . '">' . $icon_code . '</a>';
...
function preyantechnosys_social_share_box($post_type='portfolio'){
	...
	$social_links = '';
	$button_html  = '';
	$portfolio_single_top_btn_title = preyantechnosys_get_option('portfolio_single_top_btn_title');
	$portfolio_single_top_btn_link  = preyantechnosys_get_option('portfolio_single_top_btn_link');
	$portfolio_social_share_title   = preyantechnosys_get_option('portfolio_social_share_title');
	...
	if( !empty($portfolio_single_top_btn_title) && !empty($portfolio_single_top_btn_title) ){
		$button_html  = '<div class="prt-single-top-btn">';
		
		$button_html .= do_shortcode('[prt-btn title="' . $portfolio_single_top_btn_title . '" style="outline" shape="square" color="black" size="sm" i_align="right" i_icon_themify="themifyicon ti-arrow-right" add_icon="true" link="url:' . urlencode(esc_url($portfolio_single_top_btn_link)) . '|||"]');
		$button_html .= '</div>';
	}
	...
	if( !empty($social_links) ){
		...
		if( $post_type=='portfolio' ){
			...
			if( !empty($portfolio_social_share_title) ){
				$return .= '<div class="prt-social-share-title">'. $portfolio_social_share_title .'</div>';
			}
...
function preyantechnosys_portfolio_related(){
	...
	if( $portfolio_show_related===true ){
		...
		$cpt           = 'portfolio'; // This will be used in preyantechnosys_get_boxes() automatically.
...
function preyantechnosys_post_class(){
	$return = '';
	$classes = array();
...
function preyantechnosys_get_post_format_icon( $post_id='' ){
	...
	if( !empty($return) ){
		$return = '<div class="prt-post-format-icon-wrapper">' . $return . '</div>';
	}
...
function preyantechnosys_create_slider($images='', $size='full'){
	...
	if( !empty($return) ){
		$return = '<div class="prt-slick-carousel-wrapper"><div class="prt-slick-carousel">' . $return . '</div></div>';
	}
...
function preyantechnosys_get_boxes( $cpt='blog', $vars=array() ){
	...
	/*} else if( $cpt == 'client' ){
		...
		$return = preyantechnosys_get_clientboxes( $vars );*/
	...
	if( !empty($return) ){
		...
		if( !empty($sortable_category) && is_array($sortable_category) && count($sortable_category)>0 ){
			...
			$sortable_category_html = '<div class="prt-sortable-wrapper prt-sortable-wrapper-' . $cpt . '"><nav class="prt-sortable-list"><ul>' . $sortable_category_html . '</ul></nav></div>';
		}
...
function preyantechnosys_global_portfolio_template_list( $for_vc=false ){
	$return = array(
		'overlay'    => esc_attr__('Content overlay on image view', 'minemo'),
		'top-image'  => esc_attr__('Top Image and Bottom Content view', 'minemo'),
	);
...
function preyantechnosys_global_blog_template_list( $for_vc=false ){
	$return = array(
		"top-image"       => esc_attr__("Top image and bottom content (default)", "minemo"),
		"left-image"      => esc_attr__('Left image and right content ', "minemo"),
		"right-image"     => esc_attr__('Right image and left content ', "minemo"),
		"content-overlay" => esc_attr__('Content overlay on image', "minemo"),
	);
...
function preyantechnosys_global_team_member_template_list( $for_vc=false ){
	$return = array(
		"overlay"    => esc_attr__("Overlay content on Image (Default)", "minemo"),
		"left-image" => esc_attr__("Left Image and Right Content", "minemo"),
	);
...
function preyantechnosys_global_client_template_list( $for_vc=false ){
	$return = array(
		'simple-logo'    => esc_attr__('Simple Logo view', 'minemo'),
		'separator-logo' => esc_attr__('Logo with Separator view', 'minemo'),
		'boxed-logo'     => esc_attr__('Boxed view', 'minemo'),
		//'logo-with-title' => esc_attr__('Logo with Title view', 'minemo'),
	);
...
function preyantechnosys_get_short_desc(){
	$blog_text_limit = preyantechnosys_get_option('blog_text_limit');
	$content = '';
	...
	if( $position=='start' ){
		...
		if ( $boxview=='carousel' ) {
			if( $vars['carousel_nav']=='above' ){
				$classArray[] = 'prt-boxes-carousel-arrows-above';
				if ( !empty( $vars['txt_align'] ) ) {
					$classArray[] = 'prt-boxes-txtalign-' . $vars['txt_align'];
				}
			} else {
				$classArray[] = 'prt-boxes-carousel-arrows-side';
			}
		}
...
function preyantechnosys_titlebar_classes(){
	...
	if( !empty($titlebar_if_bg_image) ){
		//$reurn[] = 'prt-titlebar-bgimage-'.$titlebar_if_bg_image;
...
function preyantechnosys_titlebar_title(){
	...
	if( is_singular() || is_home() ){  // single page, single post and single cpt
		...
		if ( have_posts() ){
			...
			$title = sprintf(
				$adv_tbar_authorarc,
				'<span class="prt-titlebar-heading prt-tbar-author-title">' . get_the_author() . '</span>'
			);
...
function preyantechnosys_topbar_content(){
	...
	if( !empty($topbar_left_text) ){
		$topbar_left_text = '<div class="prt-wrap-cell">'. do_shortcode($topbar_left_text) .'</div>';
	}
...
function preyantechnosys_header_links(){
	...
	if( preyantechnosys_fbar_show()==true ){
		...
		<span class="preyantechnosys-fbar-btn ' . preyantechnosys_sanitize_html_classes(preyantechnosys_fbar_btn_classes()) . '">
			<a href="javascript:void(0);" class="preyantechnosys-fbar-btn-link prt-icolor-' . preyantechnosys_sanitize_html_classes( preyantechnosys_get_option('fbar_icon_color') ) . ' prt-bgcolor-' . preyantechnosys_sanitize_html_classes( preyantechnosys_get_option('fbar_btn_bg_color') ) . '">
	...
	if( in_array($header_style, array('infostack','infostack-rtl','infostack-overlay','infostack-overlay-rtl' ) ) ){
		$header_btn_url  = preyantechnosys_get_option('header_btn_url');
		$header_btn_text = preyantechnosys_get_option('header_btn_text');
		if( !empty($header_btn_url) && !empty($header_btn_text) ){
			$return  .= '<span class="prt-header-icon prt-header-btn-w"><a href="' . esc_url($header_btn_url) . '">' . esc_attr($header_btn_text) . '</a></span>';
		}
	...
	if( function_exists('is_woocommerce') && $wc_header_icon==true ){
		...
		$return  .= '<span class="prt-header-icon prt-header-wc-cart-link"><a href="' . $cart_url . '"><i class="prt-minemo-icon-cart"></i><span class="number-cart">&nbsp;</span></a></span>';
	}
	...
	if( $header_search==true ){
		$class   = '';
		$return .= '<span class="prt-header-icon prt-header-search-link"><a href="javascript:void(0);"><i class="prt-minemo-icon-search"></i></a></span>';
	}
...
function preyantechnosys_one_page_site_js(){
	...
	<script type="text/javascript">
		var x = 1;
		jQuery('.mega-menu a, .menu-prt-main-menu-container a').each(function(){
			if( x != 1 ){
...
function preyantechnosys_header_style_class( $echo=false ){
	...
	if (strpos( preyantechnosys_get_headerstyle(), 'rtl') !== false) {
...
function preyantechnosys_fbar_btn(){
	$return = '<!-- Open/close button -->
			<span class="preyantechnosys-fbar-btn ' . preyantechnosys_sanitize_html_classes(preyantechnosys_fbar_btn_classes()) . '">
				<a href="javascript:void(0)" class="preyantechnosys-fbar-btn-link">
					' . preyantechnosys_fbar_open_icon() . '
					' . preyantechnosys_fbar_close_icon() . '
					<span class="prt-hide">' . esc_attr__('Open', 'minemo') . '</span>
				</a>
...
function preyantechnosys_fbar_open_icon(){
	...
	$return = '<span class="prt-fbar-open-icon">' . preyantechnosys_create_icon_from_data( $fbar_handler_icon, true ) . '</span>';
...
function preyantechnosys_fbar_close_icon(){
	...
	$return = '<span class="prt-fbar-close-icon" style="display:none;">' . preyantechnosys_create_icon_from_data( $fbar_handler_icon_close, true ) . '</span>';
...
function preyantechnosys_fbar_close_icon_for_content_area(){
	...
	$fbar_handler_icon_close = preyantechnosys_get_option('fbar_handler_icon_close');
	$return = preyantechnosys_create_icon_from_data( $fbar_handler_icon_close, true );
...
//add_action( 'wp_enqueue_scripts', 'preyantechnosys_floatingbar_inline_css', 16 );
...
function preyantechnosys_box_team_social_links(){
	...
	if( !empty($data) && is_array($data) && count($data)>0 ){
		...
		foreach($data as $social ){
			...
			$return .= '<li><a href="'. $social['social_icons_list_link'] .'" target="_blank"><i class="prt-minemo-icon-'. $social['social_icons_list_icon'] .'"></i><span class="prt-hide">'. $social_name .'</span></a></li>';
		}
...
function preyantechnosys_testimonial_title(){
	...
	$return .= ( !empty($clienturl) ) ? '<span class="preyantechnosys-author-name"><a href="'.esc_url($clienturl).'" target="_blank">'.get_the_title().'</a></span>' : '<span class="preyantechnosys-author-name">'.get_the_title().'</span>' ;
 ...
 *
 * Create your own preyantechnosys_header_button() to override in a child theme.
 *
 * @since Minemo 1.0
 *
 */
...
function preyantechnosys_header_text(){
	...
	if( in_array( $headerstyle, $valid_headerstyle ) && !empty( $header_text ) ){
		...
		$return      = '<div class="prt-header-text-area">'.do_shortcode( $header_text ).'</div>';
	}
...
function preyantechnosys_get_sidebar_info(){
	...
	$return .= '<div class="preyantechnosys-meta-details preyantechnosys-event-meta-details">';
		$return .= '<span class="preyantechnosys-event-meta-item preyantechnosys-event-date"> ';
			...
			if ( tribe_event_is_all_day() && tribe_event_is_multiday() ){
				...
				$return .= '
					<span class="preyantechnosys-event-meta-dtstart" title="' . esc_attr( $start_ts ) . '"> ' .  esc_attr( $start_date ) . ' </span> - 
			...
			// Multiday events
				...
				$return .= '<span class="preyantechnosys-event-meta-dtstart" title="' . esc_attr( $start_ts ) . '"> ' . esc_attr( $start_datetime ) . ' </span> - ';
			...
			// Single day events
				...
				$return .= '<span class="preyantechnosys-event-meta-dtstart" title="' . esc_attr( $start_ts ) . '"> ' . esc_attr( $start_date ) . ' </span> - ';
		...
		$return .=' </span>';
		$return .= '
			&nbsp;&nbsp; <span class="preyantechnosys-event-meta-item preyantechnosys-event-price">
				'.$price.'
			</span>';
	$return .= '</div>';
	return $return;
...
function preyantechnosys_event_description(){ 
	$return   = '';
	$readMore = esc_attr__('See Event', 'minemo') . ' <i class="kwicon-fa-angle-right"></i>';
...
function preyantechnosys_comment_row_template($comment, $args, $depth){
    ...
    <div class="comment-meta commentmetadata">
		<?php printf( esc_attr__( '<cite class="prt-comment-owner fn">%s</cite> <span class="says">says:</span>', 'minemo' ), get_comment_author_link() ); ?>
 ...
 */
if( !function_exists('preyantechnosys_404_icon') ){
function preyantechnosys_404_icon(){
	$icon   = preyantechnosys_get_option('error404_big_icon');
	$return = ( !empty($icon['library_' . $icon['library']] ) ) ? '<div class="prt-big-icon"><i class="' . $icon['library_' . $icon['library']] . '"></i></div>' : '' ;
	return $return;
}
}
...
function preyantechnosys_search_results_content(){
		...
		switch( $post_type ){
			...
			case 'page' :
				if( $curr_total_page <= $total_page ){
					...
					$data = preyantechnosys_enc_data('<a href="' . get_permalink() . '">' . get_the_title() . '</a>');
	...
	if( is_array($data_page) && count($data_page)>0 ){
		...
		if( !empty($_GET['post_type']) && $_GET['post_type']=='page' ){
			...
			$data_page_html .= do_shortcode('[prt-list icon_icon_fontawesome="fa fa-file-text-o" ' . $shortcode1 . ']');
			$data_page_html .= '</div>';
			$data_page_html .= '<div class="col-sm-6">';
			$data_page_html .= do_shortcode('[prt-list icon_icon_fontawesome="fa fa-file-text-o" ' . $shortcode2 . ']');
			$data_page_html .= '</div>';
			...
			$data_page_html .= do_shortcode('[prt-list icon_icon_fontawesome="fa fa-file-text-o" ' . $shortcode . ']');
...
function preyantechnosys_search_results_box_title( $post_type='post' ){
	...
	if( !empty($singular_name) ){
		...
		$return .= '<div class="prt-sresults-title-w"><h2 class="prt-sresults-title">' . sprintf(
				esc_attr__('Search results for %s','minemo'),
 ...
 */
if( !function_exists('preyantechnosys_responsive_padding_margin') ){
function preyantechnosys_responsive_padding_margin( $data='' , $parent_class='' ){
	...
	if( !empty($data) ){
		...
		if( !empty($css_1200)   ){ $return .= '@media (max-width: 1200px){ '.$parent_class.'.prt-responsive-custom-'.$class.'{'.$css_1200.'} }'; }
		if( !empty($css_991)    ){ $return .= '@media (max-width: 991px ){ '.$parent_class.'.prt-responsive-custom-'.$class.'{'.$css_991.'} }'; }
		if( !empty($css_767)    ){ $return .= '@media (max-width: 767px ){ '.$parent_class.'.prt-responsive-custom-'.$class.'{'.$css_767.'} }'; }
		if( !empty($css_custom) ){ $return .= '@media (max-width: '.$custom_width.' ){ '.$parent_class.'.prt-responsive-custom-'.$class.'{'.$css_custom.'} }'; }
