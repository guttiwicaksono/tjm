 ...
 * @since 1.0
 */
class minemo_flicker_widget extends WP_Widget {
	...
	function __construct() {
		$widget_style = array('classname'   => 'minemo_flicker_widget',
							  'description' => esc_attr__('Show photos from Flickr.','minemo') );
		...
		parent::__construct(
			'minemo_flicker_widget', // Base ID
			esc_attr__('Minemo Flickr Widget', 'minemo'), // Name
			$widget_style // Args
		);
	}
	...
	function widget( $args, $cur_instance ) {
		...
		echo '<div class="minemo_flicker_widget_wrapper">'; ?>
	...
	function form( $cur_instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'title' )); ?>"><?php echo esc_attr__('Title', 'minemo'); ?>:</label>
...
register_widget( 'minemo_flicker_widget' );